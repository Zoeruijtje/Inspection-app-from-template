...
type StubEntry = { typeId: string; defaultConfig: RegistryEntry["defaultConfig"] };
...
describe("block registry baseline", () => {
  it("2. contains exactly the four baseline block types", () => {
    const ids = blockRegistry.list().map((d) => d.typeId).sort();
...
describe("registry factory", () => {
  ...
  it("5b. accepts a non-duplicate set", () => {
    ...
    expect(reg.list().map((d) => d.typeId).sort()).toEqual(["a", "b"]);
  });
});
...
describe("config validation — valid samples pass", () => {
  it("7a. section valid configs pass", () => {
    ...
    expect(schema.safeParse({ collapsible: false, initiallyCollapsed: false }).success).toBe(true);
    expect(schema.safeParse({ collapsible: true, initiallyCollapsed: false }).success).toBe(true);
    expect(schema.safeParse({ collapsible: true, initiallyCollapsed: true }).success).toBe(true);
    expect(schema.safeParse(containerRegistry.require("section").defaultConfig).success).toBe(true);
  });
  ...
  it("7b. heading valid configs pass", () => {
    ...
    expect(schema.safeParse({ level: 4, text: "x".repeat(500) }).success).toBe(true);
    expect(schema.safeParse(blockRegistry.require("heading").defaultConfig).success).toBe(true);
  });
  ...
  it("7c. paragraph valid configs pass", () => {
    ...
    expect(schema.safeParse(blockRegistry.require("paragraph").defaultConfig).success).toBe(true);
  });
  ...
  it("7d. short_text valid configs pass", () => {
    ...
    expect(schema.safeParse({ maxLength: 100, placeholder: "p", defaultValue: "" }).success).toBe(true);
    expect(schema.safeParse({ maxLength: 2000, defaultValue: "x".repeat(2000) }).success).toBe(true);
    expect(schema.safeParse(blockRegistry.require("short_text").defaultConfig).success).toBe(true);
  });
  ...
  it("7e. single_select valid configs pass", () => {
    ...
    expect(schema.safeParse({ allowOther: true, otherLabel: "Other" }).success).toBe(true);
    expect(schema.safeParse({ allowOther: true, otherLabel: "Please specify", defaultValue: "a" }).success).toBe(true);
    expect(schema.safeParse(blockRegistry.require("single_select").defaultConfig).success).toBe(true);
  });
});
...
describe("config validation — invalid configs fail", () => {
  it("8a. section rejects initiallyCollapsed:true when collapsible:false", () => {
    const schema = containerRegistry.require("section").configSchema;
    expect(schema.safeParse({ collapsible: false, initiallyCollapsed: true }).success).toBe(false);
  });
  ...
  it("8b. heading rejects invalid level/empty/oversized text", () => {
    ...
    expect(schema.safeParse({ level: 1, text: "x".repeat(501) }).success).toBe(false);
  ...
  it("8d. short_text rejects bad maxLength/placeholder types and bounds", () => {
    ...
    expect(schema.safeParse({ maxLength: 10, placeholder: "x".repeat(201) }).success).toBe(false);
  });
  ...
  it("8e. single_select rejects bad types and oversized defaultValue", () => {
    ...
    expect(schema.safeParse({ defaultValue: "x".repeat(121) }).success).toBe(false);
    expect(schema.safeParse({ allowOther: true, otherLabel: "x".repeat(201) }).success).toBe(false);
  });

  it("11. short_text rejects a default value longer than maxLength", () => {
    const schema = blockRegistry.require("short_text").configSchema;
    expect(schema.safeParse({ maxLength: 3, defaultValue: "abcd" }).success).toBe(false);
  });
  ...
  it("12. single_select rejects allowOther:true without a valid otherLabel", () => {
    ...
    expect(schema.safeParse({ allowOther: true, otherLabel: "   " }).success).toBe(false);
    expect(schema.safeParse({ allowOther: true, otherLabel: "" }).success).toBe(false);
  });
});
...
describe("strict schemas reject unknown properties", () => {
  it("section config rejects unknown properties", () => {
    const schema = containerRegistry.require("section").configSchema;
    expect(
      schema.safeParse({ collapsible: false, initiallyCollapsed: false, surprise: 1 }).success,
    ).toBe(false);
  });

  it("heading config rejects unknown properties", () => {
    const schema = blockRegistry.require("heading").configSchema;
    expect(schema.safeParse({ level: 1, text: "x", surprise: 1 }).success).toBe(false);
  });

  it("short_text config rejects unknown properties", () => {
    const schema = blockRegistry.require("short_text").configSchema;
    expect(schema.safeParse({ maxLength: 100, surprise: 1 }).success).toBe(false);
  });

  it("single_select config rejects unknown properties", () => {
    const schema = blockRegistry.require("single_select").configSchema;
    expect(schema.safeParse({ allowOther: false, surprise: 1 }).success).toBe(false);
  });
...
describe("response schemas", () => {
  ...
  it("10. input-block response schemas accept valid responses and reject malformed ones", () => {
    ...
    expect(shortText.safeParse({ value: "x".repeat(2001) }).success).toBe(false);
    ...
    expect(singleSelect.safeParse({ value: "x".repeat(121) }).success).toBe(false);
  });
});

describe("registry immutability", () => {
  it("13. returned values are frozen and cannot mutate internal registry state", () => {
    const blockSnapshot = blockRegistry.list().map((d) => d.typeId).sort();
    ...
    expect(() => {
      (blockDef.pdfPaginationContract as unknown as Record<string, unknown>).splittable = true;
    }).toThrow();

    const blockList = blockRegistry.list();
    expect(Object.isFrozen(blockList)).toBe(true);
    expect(() => {
      (blockList as unknown as Array<typeof blockList[number]>).push(blockList[0]);
    }).toThrow();

    expect(blockRegistry.list().map((d) => d.typeId).sort()).toEqual(blockSnapshot);
    ...
    expect((freshBlock.defaultConfig as Record<string, unknown>).injected).toBeUndefined();

    const containerSnapshot = containerRegistry.list().map((d) => d.typeId).sort();
    ...
    expect(() => {
      (sectionDef.reportLayoutContract as unknown as Record<string, unknown>).splittable = false;
    }).toThrow();
    expect(() => {
    ...
    expect(() => {
      (containerList as unknown as Array<typeof containerList[number]>).push(containerList[0]);
    }).toThrow();

    expect(containerRegistry.list().map((d) => d.typeId).sort()).toEqual(containerSnapshot);
    const freshSection = containerRegistry.require("section");
    expect(freshSection.typeId).toBe("section");
    expect((freshSection.defaultConfig as Record<string, unknown>).injected).toBeUndefined();
  });
});

afterAll(() => {
  const cIds = containerRegistry.list().map((d) => d.typeId);
  const bIds = blockRegistry.list().map((d) => d.typeId).sort();
  console.log(
    `\nRegistry inventory — containers: ${cIds.length} [${cIds.join(", ")}] | blocks: ${bIds.length} [${bIds.join(", ")}]`,
  );
});
