...
describe('block compatibility helpers', () => {
	it('allows current baseline blocks inside section containers', () => {
		...
		expect(() =>
			assertBlockContainerCompatibility(block, section),
		).not.toThrow();
	});
...
function containerDef(
	...
	return {
		...
		acceptsBlocks: true,
		builderComponent: (() =>
			null) as ContainerTypeDefinition['builderComponent'],
		runtimeComponent: (() =>
			null) as ContainerTypeDefinition['runtimeComponent'],
...
function blockDef(
	overrides: Partial<BlockTypeDefinition>,
): BlockTypeDefinition {
	return {
		typeId: 'heading',
		label: 'Heading',
		...
		builderPreviewComponent: (() =>
			null) as BlockTypeDefinition['builderPreviewComponent'],
