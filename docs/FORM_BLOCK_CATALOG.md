...
This catalogue defines every block type available in the form builder platform. Each block type must be registered in the controlled block registry with the following 10 required properties:
...
Each block entry below specifies: typeId, label, category, description, phase, config properties, response type, supported containers, repeatable flag, and pdfPaginationContract.

---

## Category 1: Structure Blocks

Blocks that define form layout and organization.
...
- **Response:** `{ value: number, currencyCode: string }`
- **Containers:** section, group, column_group, repeating_group
- **Repeatable:** Yes
- **Pagination:** `{ splittable: false, keepTogether: true }`
...
- **Response:** `{ amountCents: number, currencyCode: string, description: string }`
- **Containers:** section, group, repeating_group
- **Repeatable:** Yes
- **Pagination:** `{ splittable: false, keepTogether: true }`
...
Blocks used exclusively in the report designer to construct report layouts.
...
Full specifications for report blocks are in `docs/REPORT_DESIGNER_PDF_SPEC.md`.
...
Every block type registered MUST provide:
